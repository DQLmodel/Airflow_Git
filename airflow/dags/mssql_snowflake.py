...
def extract_data_from_mssql(**context):
    """Extract retail sales data from MSSQL and save to CSV."""
    ...
    try:
        ...
        if len(df) == 0:
            ...
            df = pd.DataFrame(
                columns=['Record_ID', 'Date', 'Store_ID', 'Product_Name', 'Category', 'Quantity', 'Unit_Price',
                         'Total_Amount', 'Customer_ID', 'Sales_Rep', 'Region'])
