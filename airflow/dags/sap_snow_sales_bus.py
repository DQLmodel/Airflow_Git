...
def extract_sales_data_from_sap():
    ...
    query = """
        SELECT 
            ...
            SO.GROSSAMOUNT AS ORDER_GROSSAMOUNT, 
            SO.NETAMOUNT AS ORDER_NETAMOUNT, 
            SO.TAXAMOUNT AS ORDER_TAXAMOUNT, 
            SO.LIFECYCLESTATUS, 
            SO.BILLINGSTATUS, 
            SO.DELIVERYSTATUS,  
            SI.SALESORDERITEM, 
            SI.PRODUCTID, 
            SI.CURRENCY AS ITEM_CURRENCY, 
            SI.GROSSAMOUNT AS ITEM_GROSSAMOUNT, 
            SI.NETAMOUNT AS ITEM_NETAMOUNT, 
            SI.TAXAMOUNT AS ITEM_TAXAMOUNT, 
            SI.ITEMATPSTATUS, 
            SI.OPITEMPOS, 
            SI.QUANTITY, 
            SI.QUANTITYUNIT, 
            SI.DELIVERYDATE,    
            BP.PARTNERID AS BUSINESSPARTNER_ID,
            BP.PARTNERROLE, 
            BP.EMAILADDRESS, 
            BP.PHONENUMBER, 
            BP.FAXNUMBER, 
            ...
            BP.CHANGEDBY AS BP_CHANGEDBY, 
            BP.CHANGEDAT AS BP_CHANGEDAT, 
            BP.CURRENCY AS BUSINESSPARTNER_CURRENCY
        FROM 
            DBADMIN.SALESORDERS SO
        INNER JOIN 
            DBADMIN.SALESORDERITEMS SI
            ON SO.SALESORDERID = SI.SALESORDERID
        LEFT JOIN 
            DBADMIN.BUSINESSPARTNERS BP
    ...
    df = pd.DataFrame(result, columns=['SALESORDERID', 'CREATEDBY', 'CREATEDAT', 'CHANGEDBY', 'CHANGEDAT', 'FISCVARIANT', 'FISCALYEARPERIOD', 'NOTEID', 'PARTNERID', 'SALESORG', 'ORDER_CURRENCY', 'ORDER_GROSSAMOUNT', 'ORDER_NETAMOUNT', 'ORDER_TAXAMOUNT', 'LIFECYCLESTATUS', 'BILLINGSTATUS', 'DELIVERYSTATUS', 'SALESORDERITEM', 'PRODUCTID', 'ITEM_CURRENCY', 'ITEM_GROSSAMOUNT', 'ITEM_NETAMOUNT', 'ITEM_TAXAMOUNT', 'ITEMATPSTATUS', 'OPITEMPOS', 'QUANTITY', 'QUANTITYUNIT', 'DELIVERYDATE', 'BUSINESSPARTNER_ID', 'PARTNERROLE', 'EMAILADDRESS', 'PHONENUMBER', 'FAXNUMBER', 'WEBADDRESS', 'ADDRESSID', 'COMPANYNAME', 'LEGALFORM', 'BP_CREATEDBY', 'BP_CREATEDAT', 'BP_CHANGEDBY', 'BP_CHANGEDAT', 'BUSINESSPARTNER_CURRENCY'])

    # Save the result to a CSV file
    df.to_csv('/tmp/sap_sales_bus.csv', index=False)

    # Close connection
    cursor.close()
    sap_conn.close()

    print("Data extracted from SAP HANA and saved to /tmp/sap_sales_bus.csv")
...
def upload_to_snowflake():
    ...
    try:
        ...
        cursor.execute(f"PUT file:///tmp/sap_sales_bus.csv @my_stage;")
        ...
        cursor.execute("""
            COPY INTO DQLABS_QA.STAGING.SALES_BUSINESS
            FROM @my_stage/sap_sales_bus.csv
...
# Initialize the DAG
with DAG(
    dag_id='sap_snow_product_sales_bus_dag',
